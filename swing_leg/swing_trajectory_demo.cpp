...
#include "swing_trajectory.hpp"   // header-only API (namespace swing)
...
int main() {
    // --- Define start/end waypoints and timing ---
    const double t0 = 0.0;
    ...
    // --- Build trajectory (AutoMid + clamped endpoints at rest) ---
    ...
    args.mid_tau      = 0.45;   // earlier peak
    args.v0.setZero();
    args.v2.setZero();
    traj.build(args);
    ...
    for (double t = t0; t <= t2 + 1e-12; t += dt) {
        ...
        T.push_back(t); P.push_back(p); V.push_back(v); A.push_back(a);
    }
